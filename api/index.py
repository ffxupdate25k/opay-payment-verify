...
def normalize_amount(value):
    """
    Normalize an amount into a consistent, comparable string form.

    Accepts ints, floats, or strings (possibly with commas, currency
    symbols, or extra whitespace) and returns a canonical string like
    "300.00", so that "300", "300.0", "300.00", and "NGN 300" (after
    stripping the symbol upstream) all compare equal.

    Returns None if the value cannot be parsed as a non-negative number.
    """
    ...
    try:
        ...
        # Strip common currency prefixes/symbols just in case
        cleaned = re.sub(
            r"^[^\d\-.]+",
            "",
            cleaned
    ...
    # Quantize to 2 decimal places so "300", "300.0", "300.00" all match
